        ...
        switch (uriMatcher.match(uri)) {
            ...
            // Make sure to notify any observers
                ...
                if(upsertCheck.moveToFirst()) {
                    ...
                    values.put(ChatProvider.ID, updatePeer.id);
                    Uri instanceUri = PeerContract.CONTENT_URI(updatePeer.id);
                    selection = ChatProvider.ID + "=";
                    String[] updateArgs = {Long.toString(updatePeer.id)};
                    db.update(PEERS_TABLE, values, selection + Long.toString(updatePeer.id), null);
                    ContentResolver cr = getContext().getContentResolver();
                    cr.notifyChange(instanceUri, null);
                    return instanceUri;
                } else {
